let ble = {
  ...
  sendCommand: function() {
    ...
    let cmd_tmp = [];
    cmd_tmp[0] = 0x80;
    cmd_tmp[1] = base.ledHex[0]//baseLED[0];
    cmd_tmp[2] = base.ledHex[1] //baseLED[1];
    let cmd_pos = 3;

    for(let i =0; i < world.block.length; i++){
      
       
      cmd_tmp[cmd_pos] = world.block[i].ledARight + (world.block[i].ledALeft << 3);  // blockLED2x2[i+blocksOffset];
